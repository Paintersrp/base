...
function TaskList({ data, list }) {
  ...
  useEffect(() => {
    if (data) {
      setCategories(
        Array.from(new Set(data.map((item) => item.task_category.name)))
      );
      setListFormData({
        title: list ? list.title : "",
        description: list ? list.description : "",
  ...
  const getTaskCountByCategory = (category) => {
    const tasksInCategory = listData.filter(
      (item) => item.task_category.name === category
    );
  ...
  listData.forEach((item, index) => {
    if (
      !lastItemByCategory[item.task_category.name] ||
      index > lastItemByCategory[item.task_category.name].index
    ) {
      lastItemByCategory[item.task_category.name] = { index, id: item.id };
    }
  });
            ...
            {categories.map((category) => (
              <div key={category}>
                ...
                <Collapse in={openCategory.includes(category)}>
                  {listData
                    .filter((item) => item.task_category.name === category)
                    .map((item, index) => {
                      const isLastItem =
