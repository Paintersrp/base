# Generated by Django 4.1.3 on 2023-03-19 19:32

import api.customs
from django.db import migrations


class Migration(migrations.Migration):

    dependencies = [
        ("general", "0002_alter_header_options_alter_header_image_and_more"),
    ]

    operations = [
        migrations.AlterField(
            model_name="header",
            name="description",
            field=api.customs.CustomTextField(
                help_text="Top Header Display of Hero Section",
                max_length=300,
                verbose_name="Description",
            ),
        ),
        migrations.AlterField(
            model_name="header",
            name="keywords",
            field=api.customs.CustomTextField(
                help_text="Top Header Display of Hero Section",
                max_length=200,
                verbose_name="Keywords",
            ),
        ),
        migrations.AlterField(
            model_name="header",
            name="page",
            field=api.customs.CustomCharField(
                help_text="Top Header Display of Hero Section",
                max_length=20,
                verbose_name="Page",
            ),
        ),
        migrations.AlterField(
            model_name="header",
            name="title",
            field=api.customs.CustomTextField(
                help_text="Top Header Display of Hero Section",
                max_length=200,
                verbose_name="Title",
            ),
        ),
        migrations.AlterField(
            model_name="header",
            name="url",
            field=api.customs.CustomURLField(
                help_text="Top Header Display of Hero Section", verbose_name="URL"
            ),
        ),
    ]
